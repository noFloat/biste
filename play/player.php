<?php session_start(); ?>
<!DOCTYPE html>
<html>
<head>
	<meta charset="UTF-8">
	<title>哔哩哔哩弹幕视频网 - ( ゜- ゜)つロ  乾杯~  - bilibili</title>
	<link href="player.css" type="text/css" rel="Stylesheet" >
    <script type="text/javascript"src="player.js" > </script>
    <script language="javascript" type="text/javascript" src="../js/top.js"></script>
	<script type="text/javascript" language="javascript" type="text/javascript" src="../js/boom.js"></script>
</head>
<body>
	<div id="tbox">
	<a id="gotop" href="#"></a>
</div>
<?php
	$site = $_POST['site'];
?>
<div class="biggest">
	<div id="first">
		<div class="zhu">
			<img src="0001.png" style="margin-top:12px;float:left">
			<div class="aa">
				<a href="../index.php">主站</a>
			</div>
		</div>
		<div class="hua">
			<a href="###">画友</a>
		</div>
		<div class="you">
			<a href="###">游戏中心</a>
		</div>
		<div class="zhi">
			<a href="###">直播</a>
		</div>
		<div class="shou">
			<a href="###">手机端</a>
		</div>
		<?php 
			if(isset($_SESSION['user'])){ ?>
				<div class="bd">
				   <div class="fg">
				    	<a href=""><?php echo $_SESSION['user']; ?></a>
				   </div>
				</div>
			<?php
			}else{?>	
				<div class="bd">
				   <div class="deng">
				   	  <div class="QQ">
				   	  	<a href="###">
				   	  		<img src="0002.png" style="margin-top:5px">
				   	  	</a>
				   	  </div>
				   	  <div class="wb">
				   	  	<a href="###">
				   	  		<img src="0003.png" style="margin-top:3px">
				   	  	</a>
				   	  </div>    
				   	  <div class="dl">
				   	  	<a href="../login.php">
				   	  		<img src="0004.png" style="margin-top:5px">
				   	  	</a>
				   	  </div>    

		           </div>
				   <div class="ce">
				    	<a href="../register.php">注册</a>
				   </div>
				</div>
			<?php } ?>
    </div>
    <div id="second"></div>
	<div id="third">
		<div class="bei">
			<img src="0005.png">
		</div>
		<ul class="dh">
		   <li class="ye">
		   	  <a href="../index.php">首页</a>	   	  
		   </li>
		   <li class="dong">
		   	  <a href="###">动画</a>
		   	  <ul id="flash" class="hidden">
		   	      <li class="f1">
		   	  	       <a href="">
		   	  	 	     <strong>>MAD-AMV</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f2">
		   	  	       <a href="">
		   	  	         <strong>>MMD.3D</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f3">
		   	  	       <a href="">
		   	  	 	     <strong>>原创.配音</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f4">
		   	  	       <a href="">
		   	  	 	     <strong>>综合</strong>
		   	  	       </a>
		   	      </li>

		      </ul>
		   </li>
		   <li class="fan">
		   	  <a href="###">番剧</a>
		   	  <ul id="every" class="hidden">
		   	      <li class="f1">
		   	  	       <a href="">
		   	  	 	     <strong>>MAD-AMV</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f2">
		   	  	       <a href="">
		   	  	         <strong>>MMD.3D</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f3">
		   	  	       <a href="">
		   	  	 	     <strong>>原创.配音</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f4">
		   	  	       <a href="">
		   	  	 	     <strong>>综合</strong>
		   	  	       </a>
		   	      </li>

		      </ul>
		   </li>
		   <li class="yin">
		   	  <a href="###">音乐</a>
		   	  <ul id="music" class="hidden">
		   	      <li class="f1">
		   	  	       <a href="">
		   	  	 	     <strong>>MAD-AMV</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f2">
		   	  	       <a href="">
		   	  	         <strong>>MMD.3D</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f3">
		   	  	       <a href="">
		   	  	 	     <strong>>原创.配音</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f4">
		   	  	       <a href="">
		   	  	 	     <strong>>综合</strong>
		   	  	       </a>
		   	      </li>

		      </ul>
		   </li>
		   <li class="wu">
		   	  <a href="###">舞蹈</a>
		   	  <ul id="dance" class="hidden">
		   	      <li class="f1">
		   	  	       <a href="">
		   	  	 	     <strong>>MAD-AMV</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f2">
		   	  	       <a href="">
		   	  	         <strong>>MMD.3D</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f3">
		   	  	       <a href="">
		   	  	 	     <strong>>原创.配音</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f4">
		   	  	       <a href="">
		   	  	 	     <strong>>综合</strong>
		   	  	       </a>
		   	      </li>

		      </ul>
		   </li>
		   <li class="xi">
		   	  <a href="###">游戏</a>
		   	  <ul id="game" class="hidden">
		   	      <li class="f1">
		   	  	       <a href="">
		   	  	 	     <strong>>MAD-AMV</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f2">
		   	  	       <a href="">
		   	  	         <strong>>MMD.3D</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f3">
		   	  	       <a href="">
		   	  	 	     <strong>>原创.配音</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f4">
		   	  	       <a href="">
		   	  	 	     <strong>>综合</strong>
		   	  	       </a>
		   	      </li>

		      </ul>
		   </li>
		   <li class="ke">
		   	  <a href="###">科技</a>
		   	  <ul id="science" class="hidden">
		   	      <li class="f1">
		   	  	       <a href="">
		   	  	 	     <strong>>MAD-AMV</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f2">
		   	  	       <a href="">
		   	  	         <strong>>MMD.3D</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f3">
		   	  	       <a href="">
		   	  	 	     <strong>>原创.配音</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f4">
		   	  	       <a href="">
		   	  	 	     <strong>>综合</strong>
		   	  	       </a>
		   	      </li>

		      </ul>
		   </li>
		   <li class="yu">
		   	  <a href="###">娱乐</a>
		   	  <ul id="fun" class="hidden">
		   	      <li class="f1">
		   	  	       <a href="">
		   	  	 	     <strong>>MAD-AMV</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f2">
		   	  	       <a href="">
		   	  	         <strong>>MMD.3D</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f3">
		   	  	       <a href="">
		   	  	 	     <strong>>原创.配音</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f4">
		   	  	       <a href="">
		   	  	 	     <strong>>综合</strong>
		   	  	       </a>
		   	      </li>

		      </ul>
		   </li>
		   <li class="gui">
		   	  <a href="###">鬼畜</a>
		   	  <ul id="chill" class="hidden">
		   	      <li class="f1">
		   	  	       <a href="">
		   	  	 	     <strong>>MAD-AMV</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f2">
		   	  	       <a href="">
		   	  	         <strong>>MMD.3D</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f3">
		   	  	       <a href="">
		   	  	 	     <strong>>原创.配音</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f4">
		   	  	       <a href="">
		   	  	 	     <strong>>综合</strong>
		   	  	       </a>
		   	      </li>

		      </ul>
		   </li>
		   <li class="ying">
		   	  <a href="###">电影</a>
		   	  <ul id="movie" class="hidden">
		   	      <li class="f1">
		   	  	       <a href="">
		   	  	 	     <strong>>MAD-AMV</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f2">
		   	  	       <a href="">
		   	  	         <strong>>MMD.3D</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f3">
		   	  	       <a href="">
		   	  	 	     <strong>>原创.配音</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f4">
		   	  	       <a href="">
		   	  	 	     <strong>>综合</strong>
		   	  	       </a>
		   	      </li>

		      </ul>
		   </li>
		   <li class="shi">
		   	  <a href="###">电视剧</a>
		   	  <ul id="long" class="hidden">
		   	      <li class="f1">
		   	  	       <a href="">
		   	  	 	     <strong>>MAD-AMV</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f2">
		   	  	       <a href="">
		   	  	         <strong>>MMD.3D</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f3">
		   	  	       <a href="">
		   	  	 	     <strong>>原创.配音</strong>
		   	  	       </a>
		   	      </li>
		   	      <li class="f4">
		   	  	       <a href="">
		   	  	 	     <strong>>综合</strong>
		   	  	       </a>
		   	      </li>

		      </ul>
		   </li>
            <button type="button" onclick="" style="position: relative;
left:242px;top:-235px;">10</button>
            <button type="button" onclick="" style="position: relative;
left:267px;top:-235px;">10</button>
            <button type="button" onclick="" style="position: relative;
left:290px;top:-235px;">10</button>
            <button type="button" onclick="" style="position: relative;
left:313px;top:-235px;">10</button>
            <button type="button" onclick="" style="position: relative;
left:336px;top:-235px;">10</button>
            <button type="button" onclick="" style="position: relative;
left:359px;top:-235px;">10</button>
            <button type="button" onclick="" style="position: relative;
left:382px;top:-235px;">10</button>
            <button type="button" onclick="" style="position: relative;
left:405px;top:-235px;">10</button>
            <button type="button" onclick="" style="position: relative;
left:430px;top:-235px;">10</button>
            <button type="button" onclick="" style="position: relative;
left:456px;top:-235px;">10</button>
            <div class="pai">
		   	  <a href="###">
		   	  	 <img src="0006.png">
		   	  	 <div class="hang">
		   	  	     <span>排行</span>
		   	  	</div>
		   	  </a>
		   </div>
		   <div class="so">
		   	  <form method="post" action="../search.php" name="search_user">
		   	      <input type="text" name="search_box" >
                  <input type="submit" value="GO!" >
              </form>       
		   </div>
		</ul>
	</div>
</div>	
<div class="back">
	<div class="title">
	   <div id="forth">
	   	    <div class="heading">
	   	    	<img src="0026.png">
	   	    	<div class="mini">
	   	    		<b>【茶菜原】愛言葉</b>
	   	    	</div>
	   	    </div>
	    	<div class="intro">
	    		<a href="test.html">主页</a>>
	    		<a href="###">音乐</a>>
	    		<a href="###">翻唱</a>
	    		<div class="time">
	    			<span>2015-02-11 18:50</span>&nbsp;&nbsp;&nbsp;
	    			<img src="0028.png">
	    			<span>20.4万</span>&nbsp;&nbsp;
	    			<img src="0029.png">
	    			<span>7673</span>&nbsp;&nbsp;
	    			<img src="0030.png">
	    			<span>8520</span>&nbsp;&nbsp;
	    			<img src="0031.png">
	    			<span>3524</span>
	    		</div>
	    	</div>
	   	    <div id="fav">
	   	    	<a href="###">
	   	    		<img src="0032.png">
	   	    	</a>
	   	    </div>
	   </div>
	   <div id="fifth">
	   	 <div class="picture"></div>
	   	 <div class="user">
	   	 	 <a id="name" href="###">天气米雪</a>
	   	 	 <br>
	   	 	 <span>哦哟哟</span>
	   	 </div>
	   	 <div class="care">
	   	 	 <a href="###">
	   	 		 <img src="0027.png">
	   	 	 </a>
	   	 </div>
	   </div>
	</div>

	 <div  class="player" id="demo">
    	   <video poster="0033.png" controls="controls" width="683px"id="Img1">
			<source src="<?php echo $site; ?>" type="video/mp4" />
			<object data="00.mp4"></object>
		</video>
    </div>
	<div class="remark">
		<div class="fi">
		    <div class="bf">
		    	<img src="0034.png">
		    </div>
		    <div class="sc">
		    	<img src="0035.png">
		    </div>
		    <div class="dm">
		    	<img src="0036.png">
		    </div>
		    <div class="sd">
		    	<img src="0037.png">
		    </div>
		    <div class="math">
		    	188<br>
		    	0<br>
		    	0<br>
		    </div>
		</div>
		<div class="se">
			<div class="wei">
				<img src="0038.png">
			</div>
			<div class="bi">
				<img src="0039.png">
			</div>
			<div class="gao">
				<img src="0040.png">
			</div>
		</div>
		<div class="thi">
			<div class="audi">
				<img src="0044.png">
				&nbsp;&nbsp;&nbsp;1501
			</div>
			<div class="coin">
				<div class="tou">
					<a href="">
						<img src="0041.png">
					</a>
				</div>
				<div class="set">
					<a href="">
						<img src="0042.png">
					</a>
				</div>
			</div>
			<div class="lishi">
				<img src="0043.png">
			</div>
		</div>
		<div class="fo">
		    <div id="jian" onmouseover="a1(this)" onmouseout="b1(this)">时间</div>
		    <div id="lun" onmouseover="a2(this)" onmouseout="b2(this)">评论</div>
		    <div id="qi" onmouseover="a3(this)" onmouseout="b3(this)">发送日期</div>
		</div>

	</div>
	<div class="control">
		<div class="yangshi">
			<img src="0045.png">
		</div>
		<div class="yanse">
			<img src="0046.png">
		</div>
		<form class="send">
			<input type="text" id="users">
			<input type="submit" value="发送>" id="sending">
		</form>
	</div>
	<div class="share">
		<div class="sharing">
		    <div class="s1">
		    		<img src="0047.png">
		    </div>
		    <div class="s2">
		    	<a href="">
		    		<img src="0048.png">
		    	</a>
		    </div>
		    <div class="s3">
		    	<a href="">
		    		<img src="0049.png">
		    	</a>
		    </div>
		    <div class="s4">
		    	<a href="">
		    		<img src="0050.png">
		    	</a>
		    </div>
		    <div class="s5">
		    	<a href="">
		    		<img src="0051.png">
		    	</a>
		    </div>
		</div>
		<div class="collect">
			<a href="">
		    	<img src="0052.png">
		    </a>
		</div>
		<div class="bp">
			<a href="">
		    	<img src="0053.png">
		    	<div class="more">
		    		<span>用手机看</span>
		    	</div>	
		    </a>
		</div>
		<div class="mark">
			<img src="0054.png">
			<div class="B">
			    <img src="0056.png">
			</div>
			<div class="zan">
			    <img src="0055.png">
			</div>
		</div>
	</div>
	<div class="tag">
	    <div class="t1">
	    	<a href="">DOTA</a>
	    </div>
	    <div class="t2">
	    	<a href="">TI</a>
	    </div>
	    <div class="t3">
	    	<a href="">撞车现场</a>
	    </div>
	</div>
	<div class="t-intro">
		 https://www.youtube.com/watch?v=ju2IS0BZHDU 讲述了从Dota到Dota2的蜕变，已经TI和Valve背后的故事。原载youtube，由Moritz “Moose” Zimmermann制作，由Natus Vincere TV（Navi）出品。中文字幕版转自优酷S[A]berShiki，中文字幕翻译S[A]berShiki。
	</div>
	<div class="conf">
		<div id="strong">
		    <strong id="strong">评论</strong>
		</div>
		<div id="check">
		    <input type="checkbox" id="confirm">
		    <img src="0057.png">
		</div>	
	</div>
	<div id="resu">
		<a onclick="onc(this)">
			<img src="0058.png">
		</a>
	</div>
	<div id="hid" style="display:none">
		<img src="0063.png">
		<form class="remark1">
			<input type="text" id="words" placeholder="请先登录">
			<input type="submit" value="发表评论" id="remarkable">
		</form>
		<div class="emotion">
			<a href="">
			   <img src="0064.png">
		    </a>
		</div>
		<div class="edit">
			<div id="e1">默认排序</div>
			<div id="e2">按赞同数</div>
			<div id="e3">按回复数</div>
			<div class="page">
				共11页
				&nbsp;<b>1</b>
				<input type="button" value="下页" id="page1">
				<input type="button" value="末页" id="page2">
			</div>
		</div>
		<div class="rv1">
			<div class="pic1">
				<img src="0065.png">
			</div>
			<div class="floor">#3</div>
			<div class="username">
				<a href="" id="usersname">坏坏小血灵</a>
			</div>
			<div class="say">太帅了 最喜欢这种视频了</div>
			<div class="bt">
				8分钟前
				<div class="move">
					<a href="" id="click">
					回复
				    </a>
			    </div>
			</div>
		</div>
		<div class="rv2">
			<div class="pic1">
				<img src="0065.png">
			</div>
			<div class="floor">#3</div>
			<div class="username">
				<a href="" id="usersname">坏坏小血灵</a>
			</div>
			<div class="say">太帅了 最喜欢这种视频了</div>
			<div class="bt">
				8分钟前
				<div class="move">
					<a href="" id="click">
					回复
				    </a>
			    </div>
			</div>
		</div>
		<div class="rv3">
			<div class="pic1">
				<img src="0065.png">
			</div>
			<div class="floor">#3</div>
			<div class="username">
				<a href="" id="usersname">坏坏小血灵</a>
			</div>
			<div class="say">太帅了 最喜欢这种视频了</div>
			<div class="bt">
				8分钟前
				<div class="move">
					<a href="" id="click">
					回复
				    </a>
			    </div>
			</div>
		</div>
	</div>
	<div class="last">
		<img src="0024.png">
	</div>
</div>
</body>	
</html>